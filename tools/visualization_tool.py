...
import cv2
import os
from util.vis_utils import compute_box_3d, project_to_image, draw_box_3d
...
class Visualizer(object):
    def __init__(
        ...
        self.out_size = 512
        self.cnt = 0
        self.show_track_color = False
        self.only_show_dots = False
        self.tango_color = False
        self.show_trace = False
        self.not_show_number = False
        self.not_show_bbox = False
    ...
    def add_coco_bbox(
        ...
        if show_class:
            if conf >= 1:
                ID = int(conf) if not self.not_show_number else ""
                txt = "{}{}".format(self.names[cat], ID) + add_txt
            else:
                txt = "{}{:.1f}".format(self.names[cat], conf) + add_txt
        else:
            if conf >= 1:
                ID = int(conf) if not self.not_show_number else ""
                txt = "{}".format(ID) + add_txt
            else:
                txt = "{:.1f}".format(conf) + add_txt
        thickness = 2
        fontsize = 0.8 if self.qualitative else 0.5
        if not self.not_show_bbox:
    ...
    def add_heatmap(self, heatmap, img_id, alpha=0.1):
        pre_image = self.imgs[img_id]
        # Rescale heatmap to a range 0-255
        heatmap = np.uint8(255 * heatmap[0] / heatmap.max())
        # heatmap = (heatmap > 150) * heatmap
        jet_heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_HOT)
        # jet_heatmap = cv2.normalize(jet_heatmap, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        ...
        jet_heatmap = F.resize(
            torch.tensor(jet_heatmap).permute(2, 0, 1),
            (pre_image.shape[0], pre_image.shape[1]),
        )
        jet_heatmap = jet_heatmap.permute(1, 2, 0)
        # Superimpose the heatmap on original image
        superimposed_img = jet_heatmap * alpha + self.imgs[img_id]
        self.imgs[img_id] = np.uint8(superimposed_img.cpu().numpy())
    ...
    def save_img(self, imgId="default", path="./cache/debug/", verbose=True):
        fp = os.path.join(path, "{}.png".format(imgId))
        cv2.imwrite(fp, self.imgs[imgId])
        if verbose:
            print("One image is aved at: ", fp)
    ...
    def save_video(self, path, name="video", fps=4):
        img_list = [self.imgs[key] for key in self.imgs]
        h, w = img_list[0].shape[0], img_list[0].shape[1]
        # fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
        fourcc = cv2.VideoWriter_fourcc(*"MJPG")
        writer = cv2.VideoWriter(os.path.join(path, name + ".avi"), fourcc, fps, (w, h))
    ...
    def add_3d_detection(
        ...
        for item in dets:
            if (
                ...
                if self.theme == "white" and not self.show_track_color:
                ...
                if loc[2] > 1:
                    ...
                    self.imgs[img_id] = draw_box_3d(
                        ...
                        cl,
                        same_color=self.show_track_color or self.qualitative,
                    )
                    if self.show_track_color or self.qualitative:
                        bbox = [
                            box_2d[:, 0].min(),
                            box_2d[:, 1].min(),
                            box_2d[:, 0].max(),
                        ...
                        self.add_coco_bbox(
                            ...
                            item["class"] - 1,
                            sc,
                            show_txt=not self.not_show_txt,
                            no_bbox=True,
                            img_id=img_id,
                        )
                    if self.show_track_color:
    ...
    def add_bird_view(self, dets, vis_thresh=0.3, img_id="bird", cnt=0):
        if self.vis_gt_bev:
            bird_view = cv2.imread(self.vis_gt_bev + "/{}bird_pred_gt.png".format(cnt))
        else:
